...
- Stock Linux/Windows are **not hard real-time**; tighten with `SCHED_FIFO` priority, a `PREEMPT_RT` kernel, or CPU isolation *after measuring*. On the hot path, keep work **bounded**: no allocation, no long locks, no blocking I/O. ([Embedded Linux](../Chapter5/intro.md) returns to this on the Pi.)
