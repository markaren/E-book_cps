...
!!! tip "Turn warnings on and take them seriously"
    Build with `-Wall -Wextra` (GCC/Clang) or `/W4` (MSVC) — set per target in [CMake](Chapter5/cmake.md). A large fraction of portability problems surface first as a warning on one compiler. Treat warnings as errors you have not hit yet.
...
The thread library is the one piece you must still wire up per platform, and [CMake](Chapter5/cmake.md) handles even that: `find_package(Threads REQUIRED)` + `target_link_libraries(... Threads::Threads)` expands to `-pthread` on Linux and nothing on Windows, so the *same* `CMakeLists.txt` links correctly everywhere ([Getting Started](getting_started.md)).
...
- **Integer sizes vary.** `int` is usually 32-bit, but `long` is 32-bit on Windows and 64-bit on 64-bit Linux. When the exact size matters — and on the wire it always does — use the **fixed-width types** from `<cstdint>`: `std::int32_t`, `std::uint16_t`, and so on.
- **Endianness varies.** The byte order of multi-byte numbers differs between architectures. Never send a number's raw bytes assuming the other end has the same order — the byte order must be *defined and agreed*. Protocol headers use big-endian ([network byte order](Chapter4/serialization.md)), as does [Modbus](Chapter4/modbus.md); serialization formats each pin down their own (protobuf and FlatBuffers little-endian, DDS/CDR a per-message flag). This is the [serialization](Chapter4/serialization.md) and [Modbus](Chapter4/modbus.md) lesson.
...
- C++ **source** is portable; **binaries** are not. [CMake](Chapter5/cmake.md) makes the *build* uniform across Windows/Linux/macOS, and [vcpkg](Chapter5/dependencies.md) supplies dependencies per platform.
...
- On the **wire**, use **fixed-width `<cstdint>` types**, mind **endianness** (agree the byte order explicitly — big-endian for protocol headers and Modbus, but each serialization format defines its own), and never send raw structs — use a [serialization format](Chapter4/serialization.md).
